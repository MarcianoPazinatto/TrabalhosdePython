...
class SquadDao:
    ...
    def listar_todos(self):
        ...
        comando_sql_select = f""" SELECT S.NOME,S.DESCRICAO,S.NUMEROPESSOAS,B.LING,A.FRAME,C.SITEMA_BANCO FROM MARCIANO_SQUAD AS S
    ...
    def buscar_por_id(self, id):
        #----- Criação do comando SQL e passado para o cursor
        comando = f""" SELECT S.NOME,S.DESCRICAO,S.NUMEROPESSOAS,B.LING,A.FRAME,C.SITEMA_BANCO FROM MARCIANO_SQUAD AS S
        INNER JOIN MARCIANO_FRAMEWORK AS A ON S.FRAMEWORKFRONTEND= A.ID
        INNER JOIN MARCIANO_LINGUAGEM AS B ON S.LINGUAGEMBACKEND = B.ID
